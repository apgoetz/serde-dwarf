...
impl<'i> DebugInfoParser<'i> {
    ...
    pub fn parse(&self) -> Result<DebugInfo> {
        ...
        for offset in &results.serialize_types {
            if let Some(name) = results.get_typename(*offset) {
                eprintln!("{}", name.0);
            }
        }
    ...
    fn get_str_val(
        ...
        if let Some(attr) = entry.attr(at)? {
            if let gimli::AttributeValue::DebugStrRef(offset) = attr.value() {
                Ok(Some(
                    self.dwarf.debug_str.get_str(offset)?.to_string()?,
                ))
            } else {
...
impl<'i> UnitParser<'i> {
    pub fn new(di_parser: &'i DebugInfoParser<'i>,
               unit: &'i gimli::Unit<Reader<'i>>) -> Self {
        UnitParser {
            di_parser,
            unit,
    ...
    pub fn parse(&mut self) -> result::Result<(), gimli::Error> {
        ...
        while let Some((delta_depth, entry)) = entries.next_dfs()? {
            ...
            //     	if let Some(name) = parser.get_str_val(entry, constants::DW_AT_name)? {
            //     	    if name == "self" {
            //     		let ptr_offset = self.get_type(entry)?;
    ...
    fn first_pass(
        ...
        match entry.tag() {
            ...
            constants::DW_TAG_subprogram => {
                if let Some(export) = self.di_parser.get_str_val(entry, constants::DW_AT_linkage_name)? {
    ...
    fn render_namespace(&mut self, cur_offset: UnitOffset<'i>) {
        ...
        let ns = if self.di_parser.options.allow_namespace(&rendered) {Some(rendered)} else {None};
...
impl<'i> UnitResults<'i> {
    ...
    fn di_to_unit(&self, offset: gimli::DebugInfoOffset) -> Option<(&gimli::Unit<Reader>, UnitOffset<'i>)> {
        ...
        if let Some((unit, u_offset)) = self.di_to_unit(offset) {
            ...
            if let Ok(entry) = unit.entry(u_offset) {
                ...
                let namespace = self.get_namespace(offset);
                    ...
                    constants::DW_TAG_array_type => Some((String::from("?array?"), "".to_string())),
                        _ => {
                            if let Ok(Some(name)) = name {
    ...
    fn deref_ptr(&self, offset: gimli::DebugInfoOffset) -> result::Result<gimli::DebugInfoOffset, gimli::Error> {
