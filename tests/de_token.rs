...
macro_rules! assert_deserialize {
    ($symname:tt, $typ:ty, $magic:expr) => {{
        ...
        let $symname =
            <$typ as Deserialize>::deserialize(MockDeserializer::new(&mut serde_result, $magic))
                .unwrap();

        let mut dwarf_result = Vec::new();
        let typ = DEBUGINFO.sym(TYPE_STR).expect(
            format!(
                "could not build Type for symbol: {} type is {}",
                TYPE_STR,
                DEBUGINFO.symtype(TYPE_STR).unwrap()
            )
            .as_str(),
        );
        typ.deserialize(MockDeserializer::new(&mut dwarf_result, $magic))
            .unwrap();
    }};
    ($symname:tt, $typ:ty) => {
