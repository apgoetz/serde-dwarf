...
impl<'de, 'a> serde::de::MapAccess<'de> for Access<'a> {
    type Error = de::value::Error;
...
impl<'de, 'a> serde::de::VariantAccess<'de> for EnumAccess<'a> {
    type Error = de::value::Error;

    fn unit_variant(self) -> Result<(), Self::Error> {
        self.calls
            .push(Tokens::UnitVariant(self.sel_variant.get().unwrap()));
        Ok(())
    }
    ...
    {
        self.calls
            .push(Tokens::NewTypeVariant(self.sel_variant.get().unwrap()));
    ...
    {
        self.calls
            .push(Tokens::TupleVariant(self.sel_variant.get().unwrap(), len));
        visitor.visit_seq(Access {
            calls: self.calls,
            len,
            magic: self.magic.saturating_sub(1),
    ...
    where
        V: Visitor<'de>,
    {
        self.calls.push(Tokens::StructVariant(
            self.sel_variant.get().unwrap(),
            fields,
        ));
        visitor.visit_seq(Access {
            calls: self.calls,
